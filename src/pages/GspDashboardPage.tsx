            ...
            <div className="p-6 bg-gradient-to-r from-kc-blue/10 via-white to-kc-blue/5">
              <div className="flex items-start justify-between gap-4">
                ...
                <div className="text-right">
                  <Badge variant={application?.status === 'submitted' ? 'default' : (application?.status === 'under_review' ? 'secondary' : 'outline')}>{(application?.status || 'draft').toUpperCase()}</Badge>
                </div>
              </div>
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <div className="text-2xl font-bold">{application?.status === 'submitted' ? 100 : (localProgress !== null ? Math.max(localProgress, application?.progressPct || 0) : (application?.progressPct || 0))}%</div>
                          </div>
                          <div className="w-1/2">
                            <Progress value={application?.status === 'submitted' ? 100 : (localProgress !== null ? Math.max(localProgress, application?.progressPct || 0) : (application?.progressPct || 0))} />
                          </div>
                        </div>
                        <div className="mt-3 text-sm text-muted-foreground">Reference: <span className="font-medium text-foreground">{application?.reference || 'Not submitted yet'}</span></div>
                      </div>
                    </div>

                    <div className="flex flex-wrap gap-3 pt-4">
                      <Button asChild variant="blue" className="rounded-full px-6 py-3">
                        <Link to="/gsp/application">{application?.status === "submitted" ? "View Application" : (!application && localProgress === null ? "Start Application" : "Continue Application")}</Link>
                      </Button>
          ...
          <Card className="rounded-3xl p-4">
            ...
            <CardContent className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <div className="font-semibold">{application?.sectionState ? Object.values(application.sectionState).filter(Boolean).length : 0}/10</div>
              </div>
              <div className="flex items-center justify-between">
                <div className="text-sm text-muted-foreground">Last saved</div>
                <div className="font-semibold">{application?.updatedAt ? new Date(application.updatedAt).toLocaleString() : '—'}</div>
              </div>
              <div className="flex items-center justify-between">
