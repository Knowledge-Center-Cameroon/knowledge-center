...
async function sendEmail(to: string, subject: string, html: string, type: string, userId?: string, meta?: any) {
...
app.post('/api/auth/register', async (req: Request, res: Response) => {
  try {
    ...
    await sendEmail(
      email,
      'Verify your KC GSP account',
      `<p>Hello ${validator.escape(name)},</p><p>Verify your email to activate your KC GSP portal account.</p><p><a href="${verifyLink}">Verify email</a></p><p>This link expires in 24 hours.</p>`,
      'auth_verify_email',
      String(user._id),
      { verifyLink },
    );
...
app.get('/api/admin/gsp/applications', async (req: Request, res: Response) => {
  ...
  if (query) {
    ...
    baseFilter.userId = { $in: users.map((u: any) => u._id) };
  }
  ...
  const merged = applications.map((a: any) => ({
    ...a,
    user: (() => {
      const user: any = userMap.get(String(a.userId));
      return user ? {
        id: String(user._id),
        name: user.name,
        email: user.email,
...
app.post('/api/blog/:postId/comments', async (req: Request, res: Response) => {
  try {
    ...
    if (!userId || !author || !content) return res.status(400).json({ error: 'User ID, author, and content are required' });
